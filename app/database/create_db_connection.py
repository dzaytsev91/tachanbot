...
def init_db(db_name: str):
    ...
    conn.execute(
        "CREATE TABLE memes_posts_v2 (id integer PRIMARY KEY, up_votes int, down_votes int, created_at timestamp,message_id int, user_id int, username string, old_hat_votes int, flood_thread_message_id int, memes_thread_message_id int, channel_message_id int, hash string, channel_up_votes int, channel_down_votes int);"
    )
    conn.execute(
