...
def main():
    ...
    query = "SELECT user_id,username, ROUND(CAST((SUM(up_votes) - SUM(down_votes) - SUM(old_hat_votes)) as float) / CAST(COUNT(*) as float), 3), COUNT(*) FROM memes_posts_v2 WHERE created_at > ? AND user_id = ? ORDER BY ROUND(CAST((SUM(up_votes) - SUM(down_votes) - SUM(old_hat_votes)) as float) / CAST(COUNT(*) as float), 3) / CAST(COUNT(*) as float) DESC"
